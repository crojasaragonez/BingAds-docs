...
You can request the token type when requesting API access, or upgrade at a later date through the [Bing Ads Developer Portal](https://developers.bingads.microsoft.com/Account). For information on requesting a developer token, see [Get Started With the Bing Ads API](/bingads/guides/get-started).
...
### Getting Customers
To get a list of the customers to which the specified user can access, call the [GetCustomersInfo](/binga/bingads/customer-management-service/getcustomersinfo) operation. The operation returns an array of objects that contain the name and identifier of each customer that matches the filter criteria that you specified, if any. To get the details of each customer in the list, such as the address or financial status, pass the customer identifier to the [GetCustomer](/binga/bingads/customer-management-service/getcustomer) operation. You can also search for customers that match a specified filter criteria using the [SearchCustomers]((/binga/bingads/customer-management-service/searchcustomers) operation.

### Updating Customers
Only a super admin or aggregator user can update customers. Because the update operation requires the time stamp of the previous write operation that was performed against the customer, you must first call the [GetCustomer](/binga/bingads/customer-management-service/getcustomersinfo) operation. The [GetCustomer](/binga/bingads/customer-management-service/getcustomersinfo) operation returns the customer's data, which includes the time stamp.
...
To update a customer, call the [UpdateCustomer](/binga/bingads/customer-management-service/updatecustomer) operation. The call will fail if another user updates the customer data between the time you called the [GetCustomer](/binga/bingads/customer-management-service/getcustomersinfo) and [UpdateCustomer](/binga/bingads/customer-management-service/updatecustomer) operations.
...
Resellers can create an account for a managed customer by calling the [SignupCustomer](/binga/bingads/customer-management-service/signupcustomer) operation. For more information, see [Adding Customers](#createcustomer).

### Getting Accounts
To get a list of the accounts that you own or manage, call the [GetAccountsInfo](/binga/bingads/customer-management-service/getaccountsinfo) operation. The operation returns an array of objects that contain the name, identifier, and status of each account. To get the details of each account in the list, such as the account's financial status, pass the account identifier to the [GetAccount](/binga/bingads/customer-management-service/getaccount) operation. You can also search for accounts that match a specified filter criteria using the [SearchAccounts]((/binga/bingads/customer-management-service/searchaccounts) operation.

For code examples that show how to search for accounts that can be managed by the current authenticated user, see [Search Accounts by User Code Example](/bingads/guides/code-example-search-user-accounts).
### Updating Accounts
Only a super admin or aggregator user can update accounts. Because the update operation requires the time stamp of the previous write operation that was performed against the account, you must first call the [GetAccount](/binga/bingads/customer-management-service/getaccount) operation. The [GetAccount](/binga/bingads/customer-management-service/getaccount) operation returns the account's data, which includes the time stamp.
...
To update an account, call the [UpdateAccount](/binga/bingads/customer-management-service/updateaccount) operation. The call will fail if another user updates the account data between the time you called the *GetAccount* and [UpdateAccount](/binga/bingads/customer-management-service/updateaccount) operations.
...
Your application must support one or more users per customer. For information on using a Microsoft Account to authenticate with Bing Ads services, see [MAuthentication with OAuth](/bingads/guides/authentication-oauth).
...
First please note that only one email address per customer can be consolidated, so in this example two@contoso.com and four@contoso.com cannot be consolidated together. Now let's see what happens after the top three users are consolidated under one@contoso.com. 
  ...
  * With Bing Ads API version 11 there is no change to access before versus after multi-user consolidation. Each of the user's access token (see [Authentication with OAuth](authentication-oauth)) represents permissions limited to the corresponding user and role. 
...
Since a recipient can accept the invitation and sign into Bing Ads with a Microsoft account different than the invitation email address, you cannot determine with certainty the mapping from [UserInvitation](/binga/bingads/customer-management-service/userinvitation) to accepted [User](/binga/bingads/customer-management-service/user). You can search by the invitation ID (returned by [SendUserInvitation](/binga/bingads/customer-management-service/senduserinvitation)), only to the extent of finding out whether or not the invitation has been accepted or has expired. The [SearchUserInvitations](/binga/bingads/customer-management-service/searchuserinvitations) operation returns all pending invitations, whether or not they have expired. Accepted invitations are not included in the [SearchUserInvitations](/binga/bingads/customer-management-service/searchuserinvitations) response.  

### Getting Users
To get a list of the users who belong to a customer, call the [GetUsersInfo](/binga/bingads/customer-management-service/getusersinfo) operation. The operation returns an array of objects that contains the log in email address and identifier of each user. To get the details of each user in the list, such as their role and account permissions in Bing Ads, call the [GetUser](/binga/bingads/customer-management-service/getuser) operation. When calling [GetUser](/binga/bingads/customer-management-service/getuser) if you leave the *UserId* element nil, the response will include details for the current authenticated user as specified by the request header credentials.

### Updating Users
Only a super admin or aggregator user can update users. Because the update operation requires the time stamp of the previous write operation that was performed against the user, you must first call the [GetUser](/binga/bingads/customer-management-service/getuser) operation. The [GetUser](/binga/bingads/customer-management-service/getuser) operation returns the user's data, which includes the time stamp.
...
To update a user's information, call the [UpdateUser](/binga/bingads/customer-management-service/updateuser) operation. The call will fail if another user updates the user data between the time you called the [GetUser](/binga/bingads/customer-management-service/getuser) and [UpdateUser](/binga/bingads/customer-management-service/updateuser) operations.
...
Only resellers can update user roles. To update a user's role, call the [UpdateUserRoles](/binga/bingads/customer-management-service/updateuserroles) operation. For users with an account role, you can add and delete the accounts that the user has access to. For users with a customer role, you can add and delete the customers that the user has access to. You can also change a user from having an account role to having a customer role or vice-versa.

### Deleting Users
Only a super admin or aggregator user can delete users. To delete a user, call the [DeleteUser](/binga/bingads/customer-management-service/deleteuser) operation. Because the delete operation requires the time stamp of the previous write operation that was performed against the user, you must first call the [GetUser](/binga/bingads/customer-management-service/getuser) operation. The [GetUser](/binga/bingads/customer-management-service/getuser) operation returns the user's data, which includes the time stamp.
...
## See Also
[Customer Management service Reference](/binga/bingads/customer-management-service/customer-management-service-reference)
