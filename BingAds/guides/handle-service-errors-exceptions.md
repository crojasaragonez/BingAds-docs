...
When a Bing Ads service operation fails, it will return a service fault e.g., the Customer Management service can return [ApiFault](/binga/bingads/customer-management-service/apifault). The fault exceptions include one or more error objects. The error objects contain the details of why the service operation failed and a code that uniquely identifies the error. For a list of error codes, see [Bing Ads Operation Error Codes](/bingads/guides/operation-error-codes).
...
|Service|Description|
|-----------|---------------|
|Ad Insight|All ad insight operations may throw [AdApiFaultDetail](/binga/bingads/ad-insight-service/adapifaultdetail) and [ApiFaultDetail](/binga/bingads/ad-insight-service/apifaultdetail).|
|Bulk|All bulk operations may throw [AdApiFaultDetail](/binga/bingads/bulk-service/adapifaultdetail) and [ApiFaultDetail](/binga/bingads/bulk-service/apifaultdetail).|
|Campaign Management|All campaign management operations may throw [AdApiFaultDetail](/binga/bingads/campaign-management-service/adapifaultdetail).<br /><br />Some campaign management operations may also throw [ApiFaultDetail](/binga/bingads/campaign-management-service/apifaultdetail) or [EditorialApiFaultDetail](/binga/bingads/campaign-management-service/editorialapifaultdetail).<br /><br />For more information, see [Campaign Management Data Objects](/binga/bingads/campaign-management-service/campaign-management-data-objects).|
|Customer Billing|All customer billing operations may throw [AdApiFaultDetail](/binga/bingads/customer-billing-service/adapifaultdetail) and [ApiFault](/binga/bingads/customer-billing-service/apifault).<br /><br />Some customer billing operations may also throw [ApiBatchFault](/binga/bingads/customer-billing-service/apibatchfault).|
|Customer Management|All customer management operations may throw [AdApiFaultDetail](/binga/bingads/customer-management-service/adapifaultdetail) and [ApiFault](/binga/bingads/customer-management-service/apifault).|
...
When you [upload](/bingads/guides/bulk-download-upload) records in a [Bulk file](/binga/bingads/bulk-service/bulk-file-schema), the upload may succeed for some records and fail for others in the batch. When you call [GetBulkUploadUrl](/binga/bingads/bulk-service/getbulkuploadurl) you can choose whether or not to receive errors in the upload results file. 

### <a name="partial-success-campaign-management"></a>Partial Success With the Campaign Management Service
For most entities, partial success is supported when calling [Campaign Management](/binga/bingads/campaign-management-service/campaign-management-service-reference) service operations. For each list index where an entity was not added, the corresponding element will be null. The *PartialErrors* element represents an array of [BatchError](/binga/bingads/campaign-management-service/batcherror) objects that contain details for any entities that were not successfully added, updated, or deleted. The list only includes a [BatchError](/binga/bingads/campaign-management-service/batcherror) for unsuccessful attempts, and does not include null elements at the index of each successfully added entity. Similarly some operations return *NestedPartialErrors* as a list of [BatchErrorCollection](/binga/bingads/campaign-management-service/batcherrorcollection), or a two dimensional [BatchError](/binga/bingads/campaign-management-service/batcherror).

> [!NOTE]
> The [ApplyProductPartitionActions](/binga/bingads/campaign-management-service/applyproductpartitionactions) operation includes *PartialErrors* in the response; however, partial success is not supported. Either the entire set of requested actions succeed and the *AdGroupCriterionIds* response list is fully populated, or all of them fail and the *PartialErrors* response list is fully populated. 
...
If you use the Bing Ads .NET [SDK](/bingads/guides/client-libraries), your application should be prepared to handle Bing Ads API [service level exceptions](#faultoverview), [WCF exceptions](#net-wcf-exceptions), and [Bing Ads .NET SDK](#net-sdk-exceptions) exceptions described below. 
...
If you use the Bing Ads Java [SDK](/bingads/guides/client-libraries), your application should be prepared to handle Bing Ads API [service level exceptions](#faultoverview) and [Bing Ads Java SDK](#java-sdk-exceptions) exceptions described below. 
...
1.  Set up the development environment as described in [Get Started Using Java with Bing Ads Services](/bingads/guides/get-started-java). 
...
2.  Edit pom.xml to include the *org.springframework* dependency. The following is a complete pom.xml example.
    ...
    ```
    > [!NOTE]
    > Version 11.5.5 of the Bing Ads Java SDK is included as an example. For details about the latest SDK dependency version, please see the [Bing Ads Java SDK GitHub README](https://github.com/BingAds/BingAds-Java-SDK).
...
If you use the Bing Ads PHP [SDK](/bingads/guides/client-libraries), your application should be prepared to handle Bing Ads API [service level exceptions](#faultoverview) and [Bing Ads PHP SDK](#php-sdk-exceptions) exceptions described below. 
...
If you use the Bing Ads Python [SDK](/bingads/guides/client-libraries), your application should be prepared to handle Bing Ads API [service level exceptions](#faultoverview) and [Bing Ads Python SDK](#python-sdk-exceptions) exceptions described below. 
...
## See Also
[Bing Ads Operation Error Codes](/bingads/guides/operation-error-codes)  
